...
def compose_current_date_url(crt_day):
    ...
    current_url = '%d-%s-%d-ora-13-00/' % (
        crt_day.day, months[crt_day.month - 1], crt_day.year)

    if (current_date in ["2020-07-01", "2020-07-02", "2020-07-03"]):
        current_url = '%02d-%s-ora-13-00/' % (crt_day.day,
                                              months[crt_day.month - 1])

    # exception treatment
    if (crt_day >= dt.datetime.strptime("2020-07-04", "%Y-%m-%d")
            or current_date == "2020-04-02"):
        current_url = '%d-%s-ora-13-00/' % (crt_day.day,
                                            months[crt_day.month - 1])
    if (current_date in ["2020-04-07", "2020-04-17", "2020-06-26"]):
        current_url = '%d-%s-%d-ora-13-00-2/' % (
            crt_day.day - 1, months[crt_day.month - 1], crt_day.year)
    ...
    if (current_date == "2020-06-29"):
        composed_url = "https://www.mai.gov.ro/22116-2/"
...
def parse_content():
    ...
    while crt_day < end_day:
        ...
        print(
            f"current date: {current_date}, table rows: {payload_table.shape[0]}, {len(tables)}"
        )
        ...
        if crt_day > dt.datetime.strptime(
                "2020-06-10", "%Y-%m-%d") and crt_day < dt.datetime.strptime(
                    "2020-06-29", "%Y-%m-%d"):
            deaths = get_deaths_incremental(paragraphs)
            print(
                f"Not processed: deaths: {deaths} previous: {previous_death}")
...
def check_number_of_counties(all_data_df):
    ...
    COUNTY_NUMBER = 42
    try:
        assert (len(all_data_df.County.unique()) == (COUNTY_NUMBER +
                                                     1)), "wrong County number"
...
def data_cleaning(all_data_df, country_data_df):
    ...
    # fix decimal point
    all_data_df['Confirmed'] = all_data_df['Confirmed'].apply(
        lambda x: fix_decimal(x))
    ...
    for feature in [
            'ati', 'quarantine', 'isolation', 'tests', 'confirmed',
            'recovered', 'deaths'
    ]:
        country_data_df[feature] = country_data_df[feature].apply(
            lambda x: fix_decimal(x))
...
def save_data(all_data_df, country_data_df):
    for date in all_data_df.Date.unique():
        ...
        d_df.to_csv(os.path.join('ro_covid_19_daily_reports', f"{date}.csv"),
                    index=False)

    all_data_df.to_csv(os.path.join('ro_covid_19_time_series',
                                    "ro_covid_19_time_series.csv"),
                       index=False)
    country_data_df.to_csv(os.path.join(
        'ro_covid_19_time_series', "ro_covid_19_country_data_time_series.csv"),
                           index=False)
